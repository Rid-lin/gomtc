...
:put "activeAddress=$activeAddress;addressL=$addressL;allowDualStackQueue=$allowDualStackQueue;clientId=$clientId;disabledL=$disabledL;insertQueueBefore=$insertQueueBefore;radiusL=$radiusL;statusL=$statusL;activeClientId=$activeClientId;addressLists=$addressLists;alwaysBroadcast=$alwaysBroadcast;commentL=$commentL;dynamicL=$dynamicL;lastSeen=$lastSeen;rateLimit=$rateLimit;useSrcMac=$useSrcMac;activeMacAddress=$activeMacAddress;agentCircuitId=$agentCircuitId;blockAccess=$blockAccess;dhcpOption=$dhcpOption;expiresAfter=$expiresAfter;leaseTime=$leaseTime;serverL=$serverL;activeServer=$activeServer;agentRemoteId=$agentRemoteId;blockedL=$blockedL;dhcpOptionSet=$dhcpOptionSet;hostName=$hostName;macAddress=$macAddress;srcMacAddress=$srcMacAddress"
}
:foreach i in=[/ip dhcp-server lease find] do={
    [:local ID [get $i id];
    [:local activeAddress [get $i active-address]];
